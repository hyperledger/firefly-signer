...
func decodeABIFixedArrayBytes(ctx context.Context, breadcrumbs string, block []byte, headStart, headPosition int, component *typeComponent) (headBytesRead int, cv *ComponentValue, err error) {
	...
	childDynamic, err := isDynamicType(ctx, component.arrayChild)
	if err != nil {
		return -1, nil, err
	}
	if childDynamic {
		...
		return walkDynamicLenArrayABIBytes(ctx, "fix", breadcrumbs, block, headStart, headPosition, component, children)
	}
